...
//             return basic_string_view(data() + __pos, __rlen);
  ...
  // strlen is not a constexpr, so a replacement has to be added here,
  // as we only need chars the other types will not be implemented
  static constexpr size_t char_strlen( const char* c ) noexcept
  {
	 return *c ? 1 + char_strlen(c + 1) : 0;
  };

  template<>
  constexpr inline
  basic_string_view<char>::basic_string_view(const char* __s)
  	  : __data(__s), __size(char_strlen(__s)) {}
}
namespace std  {
