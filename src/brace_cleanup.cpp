...
static void print_stack(log_sev_t logsev, const char *str, const parse_frame_t &frm);
...
static size_t preproc_start(parse_frame_t &frm, chunk_t &pc)
{
   ...
   const chunk_t *next = chunk_get_next_ncnl(&pc);
   if (next == nullptr)
   {
      return(cpd.pp_level);
   }
   ...
   const size_t pp_level = (cpd.in_preproc == CT_PP_DEFINE)
                           ? cpd.pp_level
                           : pf_check(&frm, &pc); // Check for #if, #else, #endif, etc
...
static bool check_complex_statements(parse_frame_t &frm, chunk_t &pc)
{
         ...
         || (frm.pse[frm.pse_tos].stage == brace_stage_e::BRACE_DO)))
   {
      ...
      {
         const c_token_t parent = frm.pse[frm.pse_tos].type;
...
{
   LOG_FUNC_ENTRY();
   chunk_t *pre_vbc = chunk_get_prev_ncnl(&pc);
   if (pre_vbc == nullptr)
   {
      return(false);
   }
   ...
   if (frm.pse[frm.pse_tos].type == CT_VBRACE_OPEN)
   {
      ...
      else
      {
         ...
         vbc = insert_vbrace_close_after(*pre_vbc, frm);
